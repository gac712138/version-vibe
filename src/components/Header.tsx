...
export function Header() {
  ...
  const [mounted, setMounted] = useState(false); // 避免 Hydration Mismatch
    ...
    async function getUserData() {
      try {
        ...
        if (user) {
          ...
          // ✅ 修正點：使用 maybeSingle() 避免 406 錯誤
          // 嘗試從 profiles 抓取，如果還沒建立 profile 就回傳 null
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-9 w-9 rounded-full p-0 border border-zinc-700 hover:border-zinc-500 focus:ring-0">
                <Avatar className="h-full w-full">
                  <AvatarImage src={profile?.avatar_url || user.user_metadata?.avatar_url} object-cover="cover" />
